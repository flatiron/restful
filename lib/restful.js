...
exports.extendRouter = function (router, resource, options) {
  ...
  router.path('/' + pluralEntity, function () {
    ...
    this.post(function () {
      ...
      //
      // Remark: In strict mode, the response header of a successful,
      // POST resource.() is expected to be 201 Created
      //
      // In non-strict mode, `restful` will instead respond with,
      // a 302 Found and it's Location header will be set to the,
      // root of the source: GET /:resource
      //
      if (router.strict) {
        code = 201;
      } else {
      ...
      resource.create(object, function (err, result) {
        return err
          ? respondWithResult(res, 500, err)
          : respondWithResult(res, code, pluralEntity, result);
      });
    });
    ...
    this.path('/' + param, function () {
      ...
      //
      if (!router.strict) {
        _extendWithNonStrictRoutes(this, resource, options);
      }
    });
  });
}
...
//
function _extendWithNonStrictRoutes(router, resource, options) {
  //
...
function respondWithResult(res, status, key, value) {
  ...
  //
  // If the response code is 302, perform a Location redirect
  //
  if (status === 302) {
