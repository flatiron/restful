  ...
  if (options.explore) {
    ...
    router.get('/', function (_id) {
      var res = this.res,
          req = this.req;
      if (!options.strict) {
        preprocessRequest(req, resources, 'list');
      }
      respond(req, res, 200, '', {});
    });
  }
    ...
    if (resource._children && resource._children.length > 0) {
      ...
      resource._children.forEach(function(child){
        ...
        if(resource._parents.length === 0) {
          clonedOptions.prefix = clonedOptions.prefix + '/' + pluralEntity + '/:id/';
        } else {
    ...
    router.path(options.prefix + '/' + pluralEntity, function () {
      ...
      this.get(function () {
        var res = this.res,
            req = this.req;
          if (!options.strict) {
            preprocessRequest(req, resource, 'list');
          }
...
function _extendWithNonStrictRoutes(router, resource, options, respond) {
  ...
  router.get(options.prefix + '/' + pluralEntity + '/find', function () {
    var res = this.res,
        req = this.req;
    preprocessRequest(req, resource, 'find');
    resource.find(req.restful.data, function(err, result){
      respond(req, res, 200, pluralEntity, result);
    });
  });

  router.post(options.prefix + '/' + pluralEntity + '/find', function () {
    var res = this.res,
        req = this.req;
    preprocessRequest(req, resource, 'find');
    resource.find(req.restful.data, function(err, result){
  ...
  router.get(options.prefix + '/' + pluralEntity + '/new', function (_id) {
    var res = this.res,
        req = this.req;
...
//
function preprocessRequest(req, resource, action, data) {
