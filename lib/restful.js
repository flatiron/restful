...
exports.extendRouter = function (router, resources, options, respond) {
  ...
  resources.forEach(function (resource) {
    ...
    if (resource._children && resource._children.length > 0) {
      ...
      //
      // Extend the router to use expose child resource as base path
      //
      exports.extendRouter(router, childResource, clonedOptions, respond);

      //
      // Also, extend the router to use expose child resource as child of parent
      //
    ...
    router.path(options.prefix + '/' + pluralEntity, function () {
      ...
      this.path('/' + param, function () {
        ...
        this.get(function (_id, childID) {
          ...
          //
          if (options.parent && typeof childID !== 'undefined') {
            _id = resource._resource.toLowerCase() + '/' + options.parent._resource.toLowerCase() + '/' + _id + '/' + childID;
          }
